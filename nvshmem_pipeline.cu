...
#define NUM_MICRO_BATCHES 1
#define MICRO_BATCH_SIZE (BATCH_SIZE / NUM_MICRO_BATCHES) // 128 / 4 = 32
...
int main(int argc, char **argv) {
  ...
  float *input = NULL;
  float *output = NULL;
  float *intermediate = NULL; // NVSHMEM symmetric memory - single large buffer
                              // for all micro-batches

  if (my_pe == 0) {
    CUDA_CHECK(cudaMalloc(&input, BATCH_SIZE * D_IN * sizeof(float)));
    // Allocate symmetric buffer large enough for ALL micro-batches
    intermediate = (float *)nvshmem_malloc(
        NUM_MICRO_BATCHES * MICRO_BATCH_SIZE * D_HIDDEN * sizeof(float));
  } else {
  ...
  for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
    if (my_pe == 0) {
      // === GPU 0: Process and Send Micro-batches with Overlap ===

      // Generate input for entire batch
      ...
      printf("[Rank 0] Iter %d: Starting %d micro-batches\n", iter,
             NUM_MICRO_BATCHES);

      // Process micro-batches with offset-based access
      for (int mb = 0; mb < NUM_MICRO_BATCHES; mb++) {
        // Calculate offset into the large symmetric buffer
        int offset = mb * MICRO_BATCH_SIZE * D_HIDDEN;
        int input_offset = mb * MICRO_BATCH_SIZE * D_IN;

        // Forward pass for this micro-batch
        forward_stage(&stage, input + input_offset, intermediate + offset,
                      MICRO_BATCH_SIZE);

        // Compute norm for validation
        float norm = compute_norm(intermediate + offset,
                                  MICRO_BATCH_SIZE * D_HIDDEN, my_pe);
        printf("[Rank 0] Iter %d, MB %d: Computed, norm=%.4f\n", iter, mb,
               norm);

        // NVSHMEM: Non-blocking put to PE 1 at same offset
        nvshmem_float_put_nbi(
            intermediate +
                offset, // dest on PE 1 (offset into symmetric buffer)
            intermediate +
                offset, // source on PE 0 (offset into symmetric buffer)
            MICRO_BATCH_SIZE * D_HIDDEN, // size of one micro-batch
            1                            // target PE
        );

        printf("[Rank 0] Iter %d, MB %d: Sent via NVSHMEM (non-blocking)\n",
               iter, mb);

        // DON'T call nvshmem_quiet() here - allow overlap!
      }

      // Wait for ALL micro-batch transfers to complete
      nvshmem_quiet();
      printf("[Rank 0] Iter %d: All micro-batches sent\n", iter);

    } else {
      // === GPU 1: Receive and Process Micro-batches ===

      printf("[Rank 1] Iter %d: Waiting for %d micro-batches\n", iter,
             NUM_MICRO_BATCHES);

      for (int mb = 0; mb < NUM_MICRO_BATCHES; mb++) {
        // Calculate offset
        int offset = mb * MICRO_BATCH_SIZE * D_HIDDEN;
        int output_offset = mb * MICRO_BATCH_SIZE * D_OUT;

        // Wait for this micro-batch to arrive
        // Simple barrier-based sync (can be optimized with signaling)
        nvshmem_barrier_all();

        // Verify data received
        float norm = compute_norm(intermediate + offset,
                                  MICRO_BATCH_SIZE * D_HIDDEN, my_pe);
        printf("[Rank 1] Iter %d, MB %d: Received, norm=%.4f\n", iter, mb,
               norm);

        // Forward pass for this micro-batch
        forward_stage(&stage, intermediate + offset, output + output_offset,
                      MICRO_BATCH_SIZE);

        float out_norm = compute_norm(output + output_offset,
                                      MICRO_BATCH_SIZE * D_OUT, my_pe);
        printf("[Rank 1] Iter %d, MB %d: Processed, norm=%.4f\n", iter, mb,
               out_norm);
      }

      // Show predictions on last iteration (from first micro-batch)
      if (iter == NUM_ITERATIONS - 1) {
        float h_output[3 * D_OUT];
        CUDA_CHECK(cudaMemcpy(h_output, output, 3 * D_OUT * sizeof(float),
                              cudaMemcpyDeviceToHost));
        printf("[Rank 1] Final iteration predictions (first 3 samples from "
               "MB0):\n");
        for (int i = 0; i < 3; i++) {
          printf("  Sample %d: [", i);
